...
contract Members is Owned {
    ...
    // (7) 会員ステイタスの追加
    function pushStatus(
        string _name, uint256 _times,
        uint256 _sum, int8 _rate) onlyOwner public {
    ...
    // (8) 会員ステイタスの内容変更
    function editStatus(
        uint256 _index, string _name,
        uint256 _times, uint256 _sum, int8 _rate) onlyOwner public {
...
contract Crowdsale is Owned {
    ...
    // (6) コンストラクタ
    function Crowdsale(
        uint _fundingGoalInEthers, uint _transferableToken,
        uint _amountOfTokenPerEther, OreOreCoin _addressOfTokenUsedAsReward) public {
    ...
    public view returns (uint min, uint shortage, uint remainToken)
    {
        if (now < deadline) {
